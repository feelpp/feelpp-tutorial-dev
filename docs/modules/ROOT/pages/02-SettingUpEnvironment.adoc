...
[source,cpp]
----
include::example$02-environment.cpp[]
----
...
[source,cpp]
----
include::example$02-environment.cfg[lines=2..3]
----

== Adding options

We pass command line options using the http://www.boost.org/doc/libs/1_53_0/doc/html/program_options.html[Boost Program Options], library using the prefix `po::` which is a {feelpp} alias for the Boost::program_options namespace. 
To add a new {feelpp} option, we must create a new  {feelpp} `options_description`. 
You must add the default {feelpp} options
and the new one that we choose here as a double value. 

NOTE: the default value will be assigned if not specified by the user.
...
It employs the MPI standard for parallelism.
